...
@compute @workgroup_size(8, 8, 1) fn tooklab(@builtin(global_invocation_id) id: vec3u) {
    ...
    let oklab = rgbToOKLab(linear.rgb);
    textureStore(img_out, id.xy, vec4f(oklab, linear.a));
}
